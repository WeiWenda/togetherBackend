...
public interface UserService {
  ...
  /**
   * @Author:weiwenda
   * @Description:获取历史参加活动,使用翻页模式或起止模式 翻页模式下，page_size表示每次显示的个数，offset 表示请求第几页数据,从0开始
  ...
  /**
   * @Author:weiwenda
   * @Description:获取同城distance'm距离内的活动 PS:异地临时组团的活动，需要大量的前期沟通，可信度不高
  ...
  /**
   * @Author: weiwenda
   * @Description: 获取用户的好友信息
   * @Date: 下午9:46 2018/6/16
   */
  List<Map<String, Object>> getFriends(int user_id);
  /**
   * @Author: weiwenda
   * @Description: 获取历史消息
   * @Date: 下午10:47 2018/6/22
   */
  List<Message> getHistoryMessage(int user_id);
  /**
  * @Author: weiwenda
  * @Description: 存储历史消息
  * @Date: 下午10:48 2018/6/22
  */
  Boolean saveMessage(int from,int to,String content);
}
